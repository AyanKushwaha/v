...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_aircraft with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
              ...
              AND c_cfg_value = 'udm.air1.11.01';
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_airport with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_aoc with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_core with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_crew with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_ctraining with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_dutytype with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_iocs with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_manpower with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_origsuffix with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_planning with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_tracking with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module air1_urm with version udm.air1.11.01'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
...
------ drop_entity_crew_link_pre_check.sql ------
PROMPT Pre-checking entity crew_link before dropping for broken references
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN NULL
            ELSE 'Entity "crew_link" is referenced from unknown entities ' || LISTAGG(c_ent_name, ', ')
                WITHIN GROUP (ORDER BY c_ent_name)
            END AS message
            FROM dave_entity_fks
            WHERE c_fk_tgtname = 'crew_link';
...
------ drop_entity_pax_link_pre_check.sql ------
PROMPT Pre-checking entity pax_link before dropping for broken references
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN NULL
            ELSE 'Entity "pax_link" is referenced from unknown entities ' || LISTAGG(c_ent_name, ', ')
                WITHIN GROUP (ORDER BY c_ent_name)
            END AS message
            FROM dave_entity_fks
            WHERE c_fk_tgtname = 'pax_link';
...
------ drop_table_crew_link_pre_check.sql ------
PROMPT Pre-checking table crew_link before dropping for unknown columns
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        ...
        SELECT CASE COUNT(*)
          WHEN 0 THEN NULL
          ELSE 'Table "crew_link" has unknown columns ' || LISTAGG(column_name, ', ')
            WITHIN GROUP (ORDER BY column_name)
          END AS message
          FROM USER_TAB_COLUMNS
          WHERE table_name = 'CREW_LINK'
          AND column_name NOT IN ('REVID', 'DELETED', 'PREV_REVID', 'NEXT_REVID', 'BRANCHID', 'UDOR2', 'ACT1', 'ACT2', 'MINCONNTIME', 'SI');
...
------ drop_table_crew_link_tmp_pre_check.sql ------
PROMPT Pre-checking table crew_link_tmp before dropping for unknown columns
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        ...
        SELECT CASE COUNT(*)
          WHEN 0 THEN NULL
          ELSE 'Table "crew_link_tmp" has unknown columns ' || LISTAGG(column_name, ', ')
            WITHIN GROUP (ORDER BY column_name)
          END AS message
          FROM USER_TAB_COLUMNS
          WHERE table_name = 'CREW_LINK_TMP'
          AND column_name NOT IN ('REVID', 'DELETED', 'PREV_REVID', 'NEXT_REVID', 'BRANCHID', 'UDOR2', 'ACT1', 'ACT2', 'MINCONNTIME', 'SI');
...
------ drop_table_pax_link_pre_check.sql ------
PROMPT Pre-checking table pax_link before dropping for unknown columns
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        ...
        SELECT CASE COUNT(*)
          WHEN 0 THEN NULL
          ELSE 'Table "pax_link" has unknown columns ' || LISTAGG(column_name, ', ')
            WITHIN GROUP (ORDER BY column_name)
          END AS message
          FROM USER_TAB_COLUMNS
          WHERE table_name = 'PAX_LINK'
          AND column_name NOT IN ('REVID', 'DELETED', 'PREV_REVID', 'NEXT_REVID', 'BRANCHID', 'UDOR2', 'ACT1', 'ACT2', 'MINCONNTIME', 'SI');
...
------ drop_table_pax_link_tmp_pre_check.sql ------
PROMPT Pre-checking table pax_link_tmp before dropping for unknown columns
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        ...
        SELECT CASE COUNT(*)
          WHEN 0 THEN NULL
          ELSE 'Table "pax_link_tmp" has unknown columns ' || LISTAGG(column_name, ', ')
            WITHIN GROUP (ORDER BY column_name)
          END AS message
          FROM USER_TAB_COLUMNS
          WHERE table_name = 'PAX_LINK_TMP'
          AND column_name NOT IN ('REVID', 'DELETED', 'PREV_REVID', 'NEXT_REVID', 'BRANCHID', 'UDOR2', 'ACT1', 'ACT2', 'MINCONNTIME', 'SI');
...
------ drop_table_crew_link_post_ddl.sql ------
PROMPT Drop table crew_link

DROP TABLE crew_link;

COMMIT;


------ drop_table_crew_link_tmp_post_ddl.sql ------
PROMPT Drop table crew_link_tmp

DROP TABLE crew_link_tmp;
...
------ drop_table_pax_link_post_ddl.sql ------
PROMPT Drop table pax_link
...
------ drop_table_pax_link_tmp_post_ddl.sql ------
PROMPT Drop table pax_link_tmp

DROP TABLE pax_link_tmp;
...
------ alter_module_air1_aircraft_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_aircraft')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_aircraft' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_aircraft', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_aircraft', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_airport_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_airport')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_airport' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_airport', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_airport', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_aoc_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_aoc')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_aoc' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_aoc', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_aoc', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_core_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_core')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_core' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_core', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_core', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_crew_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_crew')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_crew' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_crew', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_crew', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_ctraining_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_ctraining')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_ctraining' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_ctraining', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_ctraining', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_dutytype_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_dutytype')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_dutytype' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_dutytype', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_dutytype', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_iocs_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_iocs')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_iocs' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_iocs', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_iocs', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_manpower_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_manpower')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_manpower' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_manpower', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_manpower', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_origsuffix_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_origsuffix')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_origsuffix' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_origsuffix', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_origsuffix', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_planning_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_planning')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_planning' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_planning', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_planning', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_tracking_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_tracking')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_tracking' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_tracking', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_tracking', 'version', 'udm.air1.11.00' );
...
------ alter_module_air1_urm_metadata.sql ------
-- Update module information metadata in dave_schemacfg
DELETE FROM dave_schemacfg
WHERE (c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'air1_urm')
   OR (c_cfg_type = 'default' AND c_cfg_group = 'air1_urm' AND c_cfg_name = 'version');
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'dave', 'module', 'air1_urm', 'udm.air1.11.00' );
INSERT INTO dave_schemacfg ( c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'default', 'air1_urm', 'version', 'udm.air1.11.00' );
...
------ alter_entity_flight_leg_metadata.sql ------
-- Update attribute description metadata in dave_attribcfg
DELETE FROM dave_attribcfg WHERE c_ent_name = 'flight_leg' AND c_attrib_name = 'ppax' AND c_cfg_type = 'default' AND c_cfg_group = 'default' and c_cfg_name = 'desc';
INSERT INTO dave_attribcfg ( c_ent_name, c_attrib_name, c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'flight_leg', 'ppax', 'default', 'default', 'desc', 'Prognosis passenger data' );
DELETE FROM dave_attribcfg WHERE c_ent_name = 'flight_leg' AND c_attrib_name = 'bpax' AND c_cfg_type = 'default' AND c_cfg_group = 'default' and c_cfg_name = 'desc';
INSERT INTO dave_attribcfg ( c_ent_name, c_attrib_name, c_cfg_type, c_cfg_group, c_cfg_name, c_cfg_value ) VALUES (
    'flight_leg', 'bpax', 'default', 'default', 'desc', 'Booked passenger data' );
...
------ drop_entity_crew_link_metadata_late.sql ------
-- Delete metadata from dave_attribcfg
DELETE FROM dave_attribcfg WHERE c_ent_name='crew_link';

-- Delete metadata from dave_entity_attribs
DELETE FROM dave_entity_attribs WHERE c_ent_name='crew_link';

-- Delete metadata from dave_entity_tables
DELETE FROM dave_entity_tables WHERE c_ent_name='crew_link';

-- Delete metadata from dave_entitycfg
DELETE FROM dave_entitycfg WHERE c_ent_name='crew_link';

-- Delete metadata from dave_entity_fks
DELETE FROM dave_entity_fks WHERE c_ent_name='crew_link';
DELETE FROM dave_entity_fks WHERE c_fk_tgtname='crew_link';

-- Delete records from dave_updated_tables
DELETE FROM dave_updated_tables WHERE tablename = 'crew_link';

-- Clear filter definitions: Delete records from dave_filter_ref
DELETE FROM dave_filter_ref
WHERE source_filter IN
    (SELECT id FROM dave_entity_filter WHERE entity = 'crew_link')
OR target_filter IN
    (SELECT id FROM dave_entity_filter WHERE entity = 'crew_link');

-- Clear filter definitions: Delete records from dave_entity_filter
DELETE FROM dave_entity_filter WHERE entity = 'crew_link';
...
------ drop_entity_pax_link_metadata_late.sql ------
-- Delete metadata from dave_attribcfg
DELETE FROM dave_attribcfg WHERE c_ent_name='pax_link';

-- Delete metadata from dave_entity_attribs
DELETE FROM dave_entity_attribs WHERE c_ent_name='pax_link';

-- Delete metadata from dave_entity_tables
DELETE FROM dave_entity_tables WHERE c_ent_name='pax_link';

-- Delete metadata from dave_entitycfg
DELETE FROM dave_entitycfg WHERE c_ent_name='pax_link';

-- Delete metadata from dave_entity_fks
DELETE FROM dave_entity_fks WHERE c_ent_name='pax_link';
DELETE FROM dave_entity_fks WHERE c_fk_tgtname='pax_link';

-- Delete records from dave_updated_tables
DELETE FROM dave_updated_tables WHERE tablename = 'pax_link';

-- Clear filter definitions: Delete records from dave_filter_ref
DELETE FROM dave_filter_ref
WHERE source_filter IN
    (SELECT id FROM dave_entity_filter WHERE entity = 'pax_link')
OR target_filter IN
    (SELECT id FROM dave_entity_filter WHERE entity = 'pax_link');

-- Clear filter definitions: Delete records from dave_entity_filter
DELETE FROM dave_entity_filter WHERE entity = 'pax_link';
