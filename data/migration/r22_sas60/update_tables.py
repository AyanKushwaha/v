...
def fixit(dc, *a, **k):
    ...
    activities.append({'id': 'OCCRM', 'si': log_types_t['si_occcrm'], 'inst': 'OXOCRM', 'grp': 'CRM', 'recurrent_typ': 'OCCRM'})
    activities.append({'id': 'CRM',   'si': log_types_t['si_crm'],    'inst': 'OX',     'grp': 'CRM', 'recurrent_typ': 'CRM'})
    activities.append({'id': 'ICRM',  'si': log_types_t['si_icrm'],                     'grp': 'CRM'})
    activities.append({'id': 'OLICRM','si': 'Instr ICRM'})
    activities.append({'id': 'OXICRM','si': 'Instr ICRM'})
    ...
    for act in activities:
        ...
        if 'grp' in act:
            ...
            grp = 'COD'
        if 'recurrent_typ' in act:
            ops.append(fixrunner.createOp('activity_set', updStat, {'id': act['id'], 'grp': grp, 'si': act['si'], 'recurrent_typ': act['recurrent_typ']}))
        else:
    ...
    # FD Attributes
    ops.append(fixrunner.createOp('training_log_set', newAct, {'typ': 'OPC FORCED',    'grp': 'SIM REC'}))
    ops.append(fixrunner.createOp('training_log_set', newAct, {'typ': 'PC FORCED',     'grp': 'SIM REC'}))
    ops.append(fixrunner.createOp('training_log_set', newAct, {'typ': 'PC CHANGE',     'grp': 'SIM REC'}))
    ops.append(fixrunner.createOp('training_log_set', newAct, {'typ': 'PC RENEWAL',    'grp': 'SIM REC'}))
    ops.append(fixrunner.createOp('training_log_set', newAct, {'typ': 'PC SKILL TEST', 'grp': 'SIM REC'}))
    ops.append(fixrunner.createOp('training_log_set', newAct, {'typ': 'ZFTT X', 'grp': 'SIM REC'}))

    ops.append(fixrunner.createOp('crew_training_t_set', newAct, {'id': 'ZFTT X', 'si': '-'}))
    # Manpower
    ops.append(fixrunner.createOp('course_block_type', newAct, {'id': 'ZFTT X', 'color': 'LightYellow', 'text': 'ZFTT X', 'resourceneed': 'DEFAULT', 'participantprod': 'Y'}))
