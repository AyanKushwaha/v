...
------ module_version_sas_establishment_pre_check.sql ------
PROMPT Pre-checking version of module sas_establishment
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN 'Expected module sas_establishment with version carmusr_jmp.01.00'
            ELSE NULL
            END AS message
            FROM dave_schemacfg
            WHERE c_cfg_type = 'dave'
              AND c_cfg_group = 'module'
              AND c_cfg_name = 'sas_establishment'
              AND c_cfg_value = 'carmusr_jmp.01.00';
...
------ drop_entity_est_snapshot_gen_config_pre_check.sql ------
PROMPT Pre-checking entity est_snapshot_gen_config before dropping for broken references
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        SELECT CASE COUNT(*)
            WHEN 0 THEN NULL
            ELSE 'Entity "est_snapshot_gen_config" is referenced from unknown entities ' || LISTAGG(c_ent_name, ', ')
                WITHIN GROUP (ORDER BY c_ent_name)
            END AS message
            FROM dave_entity_fks
            WHERE c_fk_tgtname = 'est_snapshot_gen_config';
...
------ drop_table_est_snapshot_gen_config_pre_check.sql ------
PROMPT Pre-checking table est_snapshot_gen_config before dropping for unknown columns
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        ...
        SELECT CASE COUNT(*)
          WHEN 0 THEN NULL
          ELSE 'Table "est_snapshot_gen_config" has unknown columns ' || LISTAGG(column_name, ', ')
            WITHIN GROUP (ORDER BY column_name)
          END AS message
          FROM USER_TAB_COLUMNS
          WHERE table_name = 'EST_SNAPSHOT_GEN_CONFIG'
          AND column_name NOT IN ('REVID', 'DELETED', 'PREV_REVID', 'NEXT_REVID', 'BRANCHID', 'CREWFILTER_NAME', 'CREWFILTER_CAT', 'HIERARCHYSETUP', 'EXPORT');
...
------ drop_table_est_snapshot_gen_config_tmp_pre_check.sql ------
PROMPT Pre-checking table est_snapshot_gen_config_tmp before dropping for unknown columns
PROMPT ================================================================================
...
DECLARE
    ...
    CURSOR check_cur IS
        ...
        SELECT CASE COUNT(*)
          WHEN 0 THEN NULL
          ELSE 'Table "est_snapshot_gen_config_tmp" has unknown columns ' || LISTAGG(column_name, ', ')
            WITHIN GROUP (ORDER BY column_name)
          END AS message
          FROM USER_TAB_COLUMNS
          WHERE table_name = 'EST_SNAPSHOT_GEN_CONFIG_TMP'
          AND column_name NOT IN ('REVID', 'DELETED', 'PREV_REVID', 'NEXT_REVID', 'BRANCHID', 'CREWFILTER_NAME', 'CREWFILTER_CAT', 'HIERARCHYSETUP', 'EXPORT');
...
------ drop_table_est_snapshot_gen_config_post_ddl.sql ------
PROMPT Drop table est_snapshot_gen_config

DROP TABLE est_snapshot_gen_config;

COMMIT;


------ drop_table_est_snapshot_gen_config_tmp_post_ddl.sql ------
PROMPT Drop table est_snapshot_gen_config_tmp

DROP TABLE est_snapshot_gen_config_tmp;
...
------ drop_module_sas_establishment_metadata.sql ------
-- Delete module information metadata from dave_schemacfg
DELETE FROM dave_schemacfg WHERE c_cfg_type = 'dave' AND c_cfg_group = 'module' AND c_cfg_name = 'sas_establishment';
DELETE FROM dave_schemacfg WHERE c_cfg_type = 'default' AND c_cfg_group = 'sas_establishment';

-- Delete module dependency metadata from dave_schemacfg
DELETE FROM dave_schemacfg WHERE c_cfg_type = 'require' AND c_cfg_group = 'sas_establishment';
...
------ drop_entity_est_snapshot_gen_config_metadata_late.sql ------
-- Delete metadata from dave_attribcfg
DELETE FROM dave_attribcfg WHERE c_ent_name='est_snapshot_gen_config';

-- Delete metadata from dave_entity_attribs
DELETE FROM dave_entity_attribs WHERE c_ent_name='est_snapshot_gen_config';

-- Delete metadata from dave_entity_tables
DELETE FROM dave_entity_tables WHERE c_ent_name='est_snapshot_gen_config';

-- Delete metadata from dave_entitycfg
DELETE FROM dave_entitycfg WHERE c_ent_name='est_snapshot_gen_config';

-- Delete metadata from dave_entity_fks
DELETE FROM dave_entity_fks WHERE c_ent_name='est_snapshot_gen_config';
DELETE FROM dave_entity_fks WHERE c_fk_tgtname='est_snapshot_gen_config';

-- Delete records from dave_updated_tables
DELETE FROM dave_updated_tables WHERE tablename = 'est_snapshot_gen_config';

-- Clear filter definitions: Delete records from dave_filter_ref
DELETE FROM dave_filter_ref
WHERE source_filter IN
    (SELECT id FROM dave_entity_filter WHERE entity = 'est_snapshot_gen_config')
OR target_filter IN
    (SELECT id FROM dave_entity_filter WHERE entity = 'est_snapshot_gen_config');

-- Clear filter definitions: Delete records from dave_entity_filter
DELETE FROM dave_entity_filter WHERE entity = 'est_snapshot_gen_config';
