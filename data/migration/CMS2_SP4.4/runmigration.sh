#!/bin/sh
#
# This script performs migration related model changes for CMS2 Service Pack 4.4
#
# Included migrations:
#

script=`basename $0`

cd `dirname $0`/..
while [ `pwd` != '/' -a ! -d "crc" ]; do
    cd ..
done
CARMUSR=`pwd`
LOG_DIR="$CARMUSR/data/migration/CMS2_SP4.4/migration_logs_`date +%Y%m%d_%H.%M.%S`"
UPDATE_SCHEMA_LOG="$LOG_DIR/database_update.log"
UPDATE_FILTERS_LOG="$LOG_DIR/database_update_filters.log"
RAVE_COMPILE_LOG="$LOG_DIR/rave_compile.log"

echo "********************************************"
echo "CMS2 SP 4.4 Database Migration script"
echo "********************************************"

echo " - Setting up ..."

echo "  * Creating log dir $LOG_DIR ..."
mkdir -p $LOG_DIR

echo "  * Setting up CARMENV ..."
. $CARMUSR/bin/carmenv.sh

cd $CARMUSR/data/migration/CMS2_SP4.4

echo "  * Compiling Tracking rulesets ..."
rave compile cct 2>&1 | tee $RAVE_COMPILE_LOG

echo " - Updating schema"
db updateschema 2>&1 | tee $UPDATE_SCHEMA_LOG

echo " - Processing migration tasks"

echo "  * SASCMS-4097 ..."
python $CARMUSR/data/migration/CMS2_SP4.4/sascms-4097.py 2>&1 | tee $LOG_DIR/SASCMS-4097.log

echo "  * SASCMS-4912 ..."
python $CARMUSR/data/migration/CMS2_SP4.4/sascms-4912.py 2>&1 | tee $LOG_DIR/SASCMS-4912.log

echo "  * SASCMS-4958 ..."
python $CARMUSR/data/migration/CMS2_SP4.4/sascms-4958.py 2>&1 | tee $LOG_DIR/SASCMS-4958.log

echo "  * SASCMS-5001 ..."
python $CARMUSR/data/migration/CMS2_SP4.4/sascms-5001.py 2>&1 | tee $LOG_DIR/SASCMS-5001.log

echo "  * SKBMM-564 ..."
cp -f $CARMUSR/crc/parameters/tracking/request_reportserver $CARMUSR/current_carmdata/RaveParameters/tracking/request_reportserver

echo " - Updating DB filters"
db updatefilters 2>&1 | tee $UPDATE_FILTERS_LOG

$CARMUSR/data/migration/CMS2_SP4.4/manpower_migration.sh

echo "********************************************"
echo "Migration finished"
echo "  Check for eventual problems in:"
echo "  $LOG_DIR"
echo "********************************************"
echo ""
