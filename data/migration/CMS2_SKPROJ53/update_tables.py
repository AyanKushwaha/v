...
def fixit(dc, *a, **k):
    ...
    log_types_t['si_tw11'] =      'AS Refresh Day'
    ...
    activities.append({'id': 'TW13', 'si': log_types_t['si_tw13'], 'inst': 'OL', 'actStat': 'UPD'}) # only update of si for trainee act
    activities.append({'id': 'TW11', 'si': log_types_t['si_tw11'], 'inst': 'OL', 'actStat': 'UPD'}) # only update of si for trainee act
    activities.append({'id': 'TW10', 'si': log_types_t['si_tw10'], 'inst': 'OL', 'actStat': 'UPD'}) # only update of si for trainee act
    ...
    activities.append({'id': 'IDS', 'si': 'Instructor day for study', 'actStat': 'UPD'}) # only update of si
    activities.append({'id': 'ISM', 'si': 'Instructor meeting', 'actStat': 'UPD'}) # only update of si
    ### CC and FD common
    activities.append({'id': 'WD', 'si': log_types_t['si_wd'], 'actStat': 'UPD'}) # only update of si
    ...
    activities.append({'id': 'E30',   'si':  log_types_t['si_e30']})
    ...
    for act in activities:
        if not 'actStat' in act:
            ...
            updStat = updAct
        ops.append(fixrunner.createOp('activity_set', updStat, {'id': act['id'], 'grp': 'COD', 'si': act['si']}))
        if 'inst' in act:
            updStat = newAct
            if len(act['inst']) == 2:
