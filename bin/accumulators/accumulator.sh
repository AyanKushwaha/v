...
build_python_eval ( )
{
    # Build string like: PythonEvalExpr("Accumulators.start('arg1', 'arg2', ...)")
    x="PythonEvalExpr(\"Accumulators.start("
    z=""
    for a in ${1+"$@"}
    do
        x="$x$z'$a'"
...
case "$ACCUMULATOR" in
    account|airport|all|ctl|installation|none|rave|ravepub|rec|trip|lifeblockhrs|new-hire)
        ;;
    specific)
        if [[ -z "$SPECIFICACCUMULATOR" ]]; then
            error "A specific accumulator was not specified (use -s 'accumulator_name')"
            exit 1
        fi
        ;;
    *)
        error "The '$ACCUMULATOR' is not a valid accumulator name."
        ;;
esac


SK_APP=Server
SK_APP_NAME=Accumulators
