...
$maxlen = 4194304;
while ($arg = shift) {
  if ($arg eq '-idm' || $arg eq '--msgid') {
    $msgid = shift;
    $msgid = decode_base64($msgid);
  }
  elsif ($arg eq '-idc' || $arg eq '--corrid') {
    $corrid = shift;
    $corrid = decode_base64($corrid);
  }
...
$gmo = {
  Version => MQGMO_VERSION_2,
  MatchOptions =>  MQMO_NONE,
  Options =>  MQGMO_CONVERT | MQGMO_ACCEPT_TRUNCATED_MSG | MQGMO_MSG_UNDER_CURSOR ,
};
$md = {
  Encoding => MQENC_NATIVE,
  CodedCharSetId => MQCCSI_Q_MGR,
...
sub print_help {
        printf STDERR "#\n";
        printf STDERR "# What:     Get one message from an MQ queue\n";
        printf STDERR "# Name:     mqget.pl\n";
        printf STDERR "# Ver:      1.2\n";
        printf STDERR "# \n";
	...
	printf STDOUT "#  -l|--maxlen bytes  [Default 4194304]\n";
        printf STDERR "#  -idm|--msgid id  [get message with id (base64-encoded)]\n";
        printf STDERR "#  -idc|--corrid id  [get message with correlation id id (base64-encoded)]\n";
        printf STDERR "#\n";
}
