...
from carmusr.tracking.PrivatelyTrade import PrivatelyTrade
...
from carmusr.tracking.informedtemptable import InformedTempTable, InformedTempTableRowCreator
...
def getLocalTime(station, utctime):
    airport = utils.airport_tz.Airport(station)
    return airport.getLocalTime(utctime)
...
class PublishInformation(RaveEvaluator):
    ...
    def split_pacts_at_month_ends(cls):
        ...
        # original end, splitting at month end(s) (midnight homebase time).
...
class RosterAttributeTableRowCreator:
    ...
    def create(self, st, et, value_str, si=None):
        rec = self.table.create((self.attr, self.crewref, st))
        rec.end_time = et
        rec.value_str = value_str
        if si:
            rec.si = str(si)
        return rec
...
def publishAll(crew_filter=0, enable_dialogs=False):
    """
    This is a substitution for the rostering-publish process,
    to be used during the test phase only.
    It is currently also used for manually publishing crew, 
    but this should be revisited as this is a remnant of old test code.

    crew_filter: 
        0 - No filter, publishes all crew
        1 - Marked/Selected crew only
        2 - Crew shown in Window 1 only

    enable_dialogs: Whether to show dialogs to the user upon calling this or not. (Confirmation, Error)
    """

    # When called via menu option, show confirmation dialog
    if enable_dialogs:
        confirm = carmstd.cfhExtensions.confirm(
            "This command will publish entire crew rosters for crew in window 1. \
            \nThis includes activities in periods marked as Do Not Publish. \
            \n\nDo you wish to continue?", 
            title="Publish Crew in Window 1")
        if not confirm:
            return 1
    ...
    if crew_filter == 2:
        crewlist = Cui.CuiGetCrew(Cui.gpc_info, Cui.CuiWhichArea, 'WINDOW')
    elif crew_filter == 1:
    ...
    if not crewlist:
        if enable_dialogs:
            Gui.GuiWarning("Publish aborted, no crew present in window")
        else:
            log("PublishAll - WARNING: publish aborted, no crew present in window")
    ...
    log("PublishAll - CuiPublishRosters: %s %s crew" % (repr([ptype for ptype in PUBLICATION_TYPES]), len(crewlist)))
    result = Cui.CuiPublishRosters(Cui.gpc_info,
                          publist,
    ...
    if result != 0:
        log("PublishAll - CuiPublishRosters: Publish potentially failed during operation.")

    UpdateManager.setDirtyTable("published_roster")
    
    Rescheduling.publish(
        crew_list=crewlist,
        start_date=start_hb,
        end_date=end_hb,
...
def setPublishedAttr(crewlist):
    ...
    p_set = t_set["PUBLISHED"]
    t_attr = TM.table("crew_attr")
    try:
        for crewid in crewlist:
            crew = TM.crew[(crewid,)]
            try:
                _attr = t_attr[(crew, AbsTime("01JAN1986"), p_set)]
            except:
                _attr = t_attr.create((crew, AbsTime("01JAN1986"), p_set))
...
def publishPreSave(dnp_period_dict=None):
    ...
    # (The opposite can of course also exist;
...
def get_revised_rosters(crewDict):
    ...
    legs = RaveIterator(RaveIterator.iter('iterators.leg_set',
                                          sort_by='leg.%start_hb%'),
                        CNF.FlightDescriptor.fields)
    rosters.link(legs)
...
def get_change_descriptions(publishedType):
    ...
    attributes_of_interest = ("st", "et", "adep", "ades", "activity", "pos")
    attribute_aliases = {"sobt": "st", "sibt": "et"}
    ...
    for crewid, diffset in diffdict.items():
        for c in sorted(diffset, key=lambda c: ct_keys.index(c.getType())):
            ...
            if table == "crew_activity":
                ...
                raise Exception("Table %s not supported" % c.getTableName)
                                                
            entity_key = entity_key.replace('\\', '')
            change = CNF.FlightDescriptor(entity_key,
                                          actCode=actcode,
                                          departureTime=dep_time,
                                          departureTimeLocal=getLocalTime(dep_stn, dep_time),
                                          departureStation=dep_stn,
                                          arrivalTime=arr_time,
                                          arrivalTimeLocal=getLocalTime(dep_stn, arr_time),
...
class AreaCrewStartEnd:
    """A simple data-class to be used with 'togglePrivatelyTraded'"""
    def __init__(self, area, crew, st, et, correct_for_privately_traded=True):
        crew_str = str(crew)
        self.area = area
        self.crew = crew_str
        # self.st = HF.hb2utc(crew_str, HF.utc2hb(crew_str, st).day_floor()) if correct_for_privately_traded else st.day_floor()
        # self.et = HF.hb2utc(crew_str, HF.utc2hb(crew_str, et).day_ceil()) if correct_for_privately_traded else et.day_ceil()
        self.st = HF.hb2utc(crew_str, st) if correct_for_privately_traded else st
        self.et = HF.hb2utc(crew_str, et) if correct_for_privately_traded else et


# Handling inform-now, privately traded and do-not-publish periods

def togglePrivatelyTraded(acse=None, freeday="Freeday", production="Production", si=None):
    """
    Called from Studio.
    Select crew and time periods that are to be marked as 'Privately Traded'.

    :param acse: Optional instance of class AreaCrewStartEnd. Overrides Studio selection / allows for setting in code.
    :param freeday: Optional string to be set
    :param production: Optional string to be set
    :param si: Optional string added to be set in 'si' field of 'roster_attr'
    :return:
    """
    def get_roster_attr_string(isFreedayAtDate, isInnerRotationsferieAtDate):
        if isFreedayAtDate:
            if isInnerRotationsferieAtDate:
                return 'Freeday Rotationsferie'
            return freeday
        return production

    def setFreedayDates(pt_dict):
        pt_dict["Freeday"].setdefault("dates", [])
        pt_dict["Production"].setdefault("dates", [])
        nr_days = int((sel.et - sel.st) / ONE_DAY)
        i = 0
        start = sel.st

        while i <= nr_days:
            date = start + i * ONE_DAY
            is_freeday = R.eval(
                R.selected('levels.chain'), 'not rescheduling.%%period_inf_any_production%%(%s, %s)' % (
                    date, date + RelTime('24:00')))[0]
            if is_freeday:
                pt_dict["Freeday"]["dates"].append(date)
            else:
                pt_dict["Production"]["dates"].append(date)
            i += 1

    def setInnerRotationsferie(pt_dict):
        nr_days = int((sel.et - sel.st) / ONE_DAY)
        start = sel.st.day_ceil()
        pt_dict["Freeday"].setdefault("inner_rot", False)

        # Get all inner rotationsferie duties that overlap the selected time span
        crew_object = HF.CrewObject(sel.crew, sel.area)
        innerRotationsferieDutyExpression = 'overlap(duty.%%start_utc%%, duty.%%end_utc%%, %s, %s) > 0:00 and duty.%%is_rotationsferie%% and freedays.%%_is_rotationsferie%%' % (
            sel.st, sel.et)
        duty_list, = crew_object.eval(R.foreach(R.iter('iterators.duty_set', where=innerRotationsferieDutyExpression), 'duty.%start_hb%', 'duty.%end_hb%'))
        nr_duties = len(duty_list)

        # Check which of the selected days overlap with an inner rotationsferie duty
        if nr_duties > 0:
            for i in xrange(nr_days):
                date_start = start + i * ONE_DAY
                date_end = date_start + RelTime('24:00')
                for j in xrange(nr_duties):
                    duty_start = duty_list[j][1]
                    duty_end = duty_list[j][2]
                    if (duty_start < date_end) and (duty_end > date_start):
                        pt_dict["Freeday"]["inner_rot"] = True
                        break

    def getFreedayStartEnd(f_min, f_max, p_min, p_max):
        if f_min >= p_max:
            start = p_max
            end = f_max
        elif f_max <= p_min:
        ...
        else:  # Should never occur, see setFreedayDates()
            start = p_min
            end = p_max
        return start, end

    def setPrivatelyTradedAttributes(attr_str, pt_dict, crew, st, et):
        print "+++ SET ATTIRBUTE STR: %s on CREW %s at st: %s , et: %s" % (attr_str, crew, st, et)
        prod_min = min(d for d in pt_dict["Production"]["dates"]) if pt_dict["Production"]["dates"] else st
        prod_max = max(d for d in pt_dict["Production"]["dates"]) if pt_dict["Production"]["dates"] else et
        fday_min = min(d for d in pt_dict["Freeday"]["dates"]) if pt_dict["Freeday"]["dates"] else prod_min
        fday_max = max(d for d in pt_dict["Freeday"]["dates"]) if pt_dict["Freeday"]["dates"] else prod_max
        fday_start, fday_end = getFreedayStartEnd(fday_min, fday_max, prod_min, prod_max)
        if attr_str == "Production":
            try:
                set_roster_attribute(_ATTR, crew, prod_min, prod_max, attr_str, si=si)
            except M.EntityError:
                # TODO: Add proper error handling for Production.
                print "setPrivatelyTradedAttributes (Production): something went wrong."
        elif attr_str == "Freeday" and pt_dict[attr_str]["inner_rot"]:
            try:
                set_roster_attribute(_ATTR, crew, fday_start, fday_end, "Freeday Rotationsferie", si=si)
            except M.EntityError:
                # TODO: Add proper error handling for Freeday Rotationsferie.
                print "setPrivatelyTradedAttributes (Freeday Rotationsferie): something went wrong."
        else:  # Freeday
            try:
                set_roster_attribute(_ATTR, crew, fday_start, fday_end, attr_str, si=si)
            except M.EntityError:
                # TODO: Add proper error handling for Freeday.
                print "setPrivatelyTradedAttributes (Freeday): something went wrong."

    def removePrivatelyTradedAttributes(attr_str, pt_dict, crew, st, et):
        if attr_str == "Freeday" and pt_dict[attr_str]["inner_rot"]:
            remove_roster_attribute(_ATTR, crew, st, et, "Freeday Rotationsferie", si)
        else:
            remove_roster_attribute(_ATTR, crew, st, et, attr_str, si)
    # -----------------------------------------------------------------------------------

    _ATTR = "PRIVATELYTRADED"

    if acse:
        sel = acse
    else:
        try:
            sel = HF.RosterDateSelection(validate_func=validate_mark_time_selection, hb=True)
            # v = RaveEvaluator(sel.area, Cui.CrewMode, sel.crew)
            # if not v.is_valid:
            #    return -1
        except Exception, e:
            log("Publish::togglePrivatelyTraded: Exception: %s" % e)
            return - 1
    correct_any_period_inconsistency(sel.area, sel.crew, si)

    try:
        pt_dict = checkPrivatelyTraded(sel.crew, sel.st, sel.et)
    except CancelException:
        return - 1

    try:
        ptd = PrivatelyTrade(sel.crew, sel.area, sel.st, sel.et)
    except:
        print "Caught unexpected error: ", sys.exc_info()[0]

    values = ["Production", "Freeday"]
    for val in values:
        if val not in pt_dict:
            pt_dict[val] = {"privately_traded": False}

    setFreedayDates(pt_dict)
    setInnerRotationsferie(pt_dict)

    for key, value in pt_dict.iteritems():
        if (key == "Production" and value["privately_traded"] is False) or (key != "Production" and value["dates"]):
            ptd.run()

            crew_object = HF.CrewObject(sel.crew, sel.area)
            duty_expression = 'overlap(duty.%%start_UTC%%, duty.%%end_UTC%%, %s, %s) > 0:00 and (duty.%%is_freeday%% or duty.%%is_vacation%%)' % (
                sel.st, sel.et)
            pact_codes, = crew_object.eval(R.foreach(R.iter('iterators.duty_set', where=duty_expression), 'duty.%code%'))

            account_list = list(set([pact_code[1] for pact_code in pact_codes]))
            AH.updateAccountsForCrewInWindow([sel.crew], account_list, sel.area)
            setPrivatelyTradedAttributes(key, pt_dict, sel.crew, sel.st, sel.et)
        elif si == "TripTrade":
            pass
        else:
            ptd.run(delete=True)
            removePrivatelyTradedAttributes(key, pt_dict, sel.crew, sel.st, sel.et)
...
def checkPrivatelyTraded(crew, st, et):
    print "Publish::checkPrivatelyTraded: crew=%s, st=%s, et=%s" % (crew, st, et)
    pt_dict = {}
    nr_days = int((et - st) / ONE_DAY)

    for row in RosterAttributeTableRowCreator("PRIVATELYTRADED", crew):
        if xrange(nr_days):
            less = row.start_time <= st
            more = row.end_time >= et
            within = ((row.start_time >= st) and (row.start_time < et)) or\
                     ((row.end_time <= et) and (row.end_time > st))
            is_production = row.value_str == "Production"

            # Production should not be updated once it has been set
            if (less and more) or (within and is_production):
                pt_dict[row.value_str] = {"privately_traded": True}
    return pt_dict


def markCancelled(currentArea):
    _ATTR = "CANCELLEDFLIGHT"
    ...
    for leg in markedLegs:
        legObject = HF.LegObject(str(leg), currentArea)
        (crewId,
         isCancelled,
         start_date,
         end_date,) = legObject.eval("crew.%id%",
                                     "leg.%is_nop%",
                                     "leg.%start_date_utc%",
                                     "leg.%end_date_utc%",)

        # Use dictionary to get rid of multiple entries for same day
        if isCancelled:
            new_entries[(crewId, start_date, end_date)] = True
            
    for key in new_entries.keys():
        se = key[2] + RelTime("24:00")
        set_roster_attribute(_ATTR, key[0], key[1], se, "Rescheduled cancelled to standby")     
...
def markInformed(add=True, acse=None, si=None):
    """
    Called from Studio.
    ...
    if add:
        add_informed_period(sel.crew, sel.st, sel.et, si)
    else:
        remove_informed_period(sel.crew, sel.st, sel.et, si)
...
class CrewPeriod:
    ...
    def addTime(self, crewid, start_time, end_time):
        if crewid not in self.start_times:
            self.start_times[crewid] = []
        self.start_times[crewid].append(start_time)
        
        if crewid not in self.end_times:
            self.end_times[crewid] = []
        self.end_times[crewid].append(end_time)
    
    def getStartTime(self, crewid):
        start_times = self.start_times[crewid]
        if start_times is not None:
            start_times.sort()
            start_time = start_times[0]
            return start_time.day_floor()
        return None
    
    def getEndTime(self, crewid):
        end_times = self.end_times[crewid]
        if end_times is not None:
            end_times.sort()
            end_time = end_times[len(end_times) - 1]
            return end_time.day_ceil()
        return None
    
    def getCrews(self):
        return self.start_times.keys()


def markMultipleInformed(add=True):
    """
    Called from Studio.
    Select legs on crew. The legs will be transformed to full days that will be 
    INFORMED at next save.
    If 'add' is False, the operation is reversed; any mark within the selected
    period will be unmarked.
    ...
    for leg in marked_legs:
        ...
        leg_start = Cui.CuiCrcEvalAbstime(Cui.gpc_info, currentArea, "object", "leg.%start_utc%")
        leg_end = Cui.CuiCrcEvalAbstime(Cui.gpc_info, currentArea, "object", "leg.%end_utc%")
        correct_any_period_inconsistency(currentArea, crewid)

        crew_periods.addTime(crewid, AbsTime(leg_start),AbsTime(leg_end))
        
    for crew in crew_periods.getCrews():
        if add:
            add_informed_period(crew, crew_periods.getStartTime(crew), crew_periods.getEndTime(crew))
        else:
            remove_informed_period(crew, crew_periods.getStartTime(crew), crew_periods.getEndTime(crew))
...
def correct_any_period_inconsistency(area, crew, si=None):
    """
...
def set_roster_attribute(attr, crew, st, et, string=None, reltime=None, abstime=None, integer=None, si=None):
    print "Publish::set_roster_attribute:", attr, st, et, "crew:", crew
    if string:  print "   string:", string
    if reltime: print "  reltime:", reltime
    if abstime: print "  abstime:", abstime
    if integer: print "  integer:", integer
    if si:      print "       si:", si
    add_period_roster_attr(RosterAttributeTableRowCreator(attr, crew), st, et, string, si)


def remove_roster_attribute(attr, crew, st, et, string=None, si=None):
    print "Publish::remove_roster_attribute:", attr, st, et, "crew:", crew
    remove_period_roster_attr(RosterAttributeTableRowCreator(attr, crew), st, et, string, si)


def roster_attribute_overlaps(attr, crew, st, et, string=None):
...
def add_informed_period(crew, st, et, si=None):
    """
    ...
    """
    print "Publish::add_informed_period:", st, et, "crew:", crew, "si:", si
    add_period(InformedTempTableRowCreator(crew), st, et, si)
    remove_do_not_publish_period(crew, st, et)
    try:
        set_time_of_briefing_update(crew, st, et)
    except:
        pass
    modcrew.add(crew)
...
def set_time_of_briefing_update(crew, st, et):
    # Find the briefing times set for legs in the period, Update the notification time.
    # Loop through all flights in period.
    Cui.CuiSetSelectionObject(Cui.gpc_info, Cui.CuiWhichArea, Cui.CrewMode, crew)
    ...
    legs = RaveIterator(R.iter('iterators.leg_set',
        ...
        "duty_check_in"       : "leg.%duty_start_utc%",
        "departure_station"   : "leg.%start_station%",
        "udor"                : "leg.%udor%",
        "flight"              : "leg.%flight_descriptor%",
        "flight_no"           : "leg.%flight_nr%"
        }).eval("default_context")
            
    for leg in legs:
        check_in_time = leg.duty_check_in
        
        if(leg.flight_no > 0 and check_in_time > st and check_in_time < et):
 
            (crewFlightDuty, numberOfPreviousNotifications) = getNumberOfNotifications(crew, leg)   
                
            """
                According to OMA16, the FDP shall start counting from the second notification, 
                so we will store that value.
            """
            if (numberOfPreviousNotifications.value_int == 0):
                numberOfPreviousNotifications.value_int = 1


def getNumberOfNotifications(crew, leg):
    adep = TM.airport[leg.departure_station]
    legRef = TM.flight_leg[leg.udor, leg.flight, adep]
    ...
    try:
        ...
        return crewFlightDuty, numberOfPreviousNotifications
    except:
        numberOfPreviousNotifications = TM.crew_flight_duty_attr.create((crewFlightDuty, briefNotifNumType))

        """
                Check new check-in change, not saved to database yet.
        """
        if modcheckin.get(crew, leg.udor, leg.flight, leg.departure_station):
            numberOfPreviousNotifications.value_int = 0
            modcheckin.remove(crew, leg.udor, leg.flight, leg.departure_station)
            return crewFlightDuty, numberOfPreviousNotifications
        else:
            raise Exception("No checkin change made")


def remove_informed_period(crew, st, et, si=None):
    """
    ...
    # HACK-TO-HANDLE-SAVEBUTTON-DIMMING-CORRECTLY: 2 lines
    delta = RelTime(1440 * 30000)
    remove_period(DoNotPublishTableRowCreator(crew), st - delta, et - delta, si)
...
def _remove_dummy_publish_periods():
    ...
    for row in itt:
        remove_period(
            DoNotPublishTableRowCreator(row.crew),
...
def add_period(creator, st, et, si=None):
    """
    Overlap handling:
    ...
    if overlap_rows:
        ...
        for row in overlap_rows:
            ...
            if _max is None or row.end_time > _max:
                _max = row.end_time
            row.remove()
        creator.create(min(st, _min), max(et, _max), si)
    else:
        creator.create(st, et, si)
    creator.reload()


def remove_period(creator, st, et, si=None):
    """
    Overlap handling:
        ...
        if st <= row.end_time and row.start_time <= et: # Overlap
            if row.end_time > et:
                ...
                if row.start_time < st:  # (A1)
                    ...
                    row.remove()
                creator.create(et, row_end_time, si)
            else:
...
def add_period_roster_attr(creator, st, et, vs="", si=None):
    """
    Overlap handling:
    ...
    if overlap_rows:
        ...
        for row in overlap_rows:
            ...
            if _max is None or row.end_time > _max:
                _max = row.end_time
            row.remove()
        creator.create(min(st, _min), max(et, _max), vs, si)
    else:
        creator.create(st, et, vs, si)
    creator.reload()


def remove_period_roster_attr(creator, st, et, vs="", si=None):
    """
    Overlap handling:
...
def insertInformedData():
    ...
    for row in itt:
        ct += 1
        InformedTableRowCreator(row.crew).create(row.start_time, row.end_time, row.si)
