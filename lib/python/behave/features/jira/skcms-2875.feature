...
Feature: Rule that controls that crew needing LPC is not assigned to same sim slot as crew needing OPC (OTS)

  Background:
    Given planning period from 1JAN2022 to 31JAN2022
  @SCENARIO1
  Scenario: Check that rule fails when LPC and OTS in same sim slot
    ...
    Then the rule "rules_training_ccr.comp_mixed_lpc_opc_or_ots_not_allowed_in_sim" shall fail on leg 1 on trip 1 on roster 1
    and rave "rules_training_ccr.%has_lpc_opc_or_ots_mismatch%" shall be "True" on leg 1 on trip 1 on roster 1

@SCENARIO2
  Scenario: Check that rule passes when both sim slots have LPC required crew
    Given planning period from 1APR2020 to 30APR2020
    ...
    Given crew member 2 has document "REC+LPC" from 1APR2018 to 31JUL2020 and has qualification "A2"
    Given crew member 2 has document "REC+OPC" from 1MAY2018 to 30SEP2020 and has qualification "A2"
    Given crew member 2 has document "REC+LPCA3" from 1APR2018 to 31MAY2020
    ...
    Then the rule "rules_training_ccr.comp_mixed_lpc_opc_or_ots_not_allowed_in_sim" shall pass on leg 1 on trip 1 on roster 1
    and rave "rules_training_ccr.%has_lpc_opc_or_ots_mismatch%" shall be "False" on leg 1 on trip 1 on roster 1
...
@SCENARIO5
  Scenario: Check that rule passes in case of SIM INSTR crew and fails for other crew with mismatch
    ...
    Then the rule "rules_training_ccr.comp_mixed_lpc_opc_or_ots_not_allowed_in_sim" shall pass on leg 1 on trip 1 on roster 1
    and rave "rules_training_ccr.%has_lpc_opc_or_ots_mismatch%" shall be "False" on leg 1 on trip 1 on roster 1
    and the rule "rules_training_ccr.comp_mixed_lpc_opc_or_ots_not_allowed_in_sim" shall fail on leg 1 on trip 1 on roster 2
    and rave "rules_training_ccr.%has_lpc_opc_or_ots_mismatch%" shall be "True" on leg 1 on trip 1 on roster 2