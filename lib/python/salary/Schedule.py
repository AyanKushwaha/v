...
from salary.Activity import SchedActivityManager
...
class ScheduleManager:
    ...
    def __init__(self, rundata, context, iterator='iterators.roster_set', crewlist=None):
        ...
        self.activityManager = SchedActivityManager(self.context, rundata, self.startDate, self.endDate)
    ...
    def getScheduleRosters(self):
        ...
        while(dt < self.endDate):
            hour_values.append("report_overtime.%%crew_scheduled_hours%%(%s)" % dt)
            dt = dt.adddays(1)
        roster_values = list(ROSTER_VALUES) + hour_values
 
        duty_iterator_where = 'not duty.%is_on_duty%'
    ...
    def createRoster(self, rosterItem):
        ...
        scheduleHours = [RelTime(0) for h in scheduleHours]
        ...
        if includeThisCrew:
            for (code, extent, startTime, endTime) in activities:
                ...
                while startTime < endTime:
                    ...
                    scheduleHours[offset] = RelTime('8:00')
                    startTime = startTime.adddays(1)

            scheduleRoster = ScheduleRoster(rosterItem[DUTIES],
                                            rosterItem[CREW_ID],
...
class ScheduleRoster(DataClass):
    """
...
class ScheduleRunDataHandler:
    ...
    def _getScheduleRun(self):
        schedule_run_query = "(&(runtype=SCHEDULE)(admcode=N)(extsys=%s)(firstdate=%s)(lastdate=%s))" %(self.rundata.extsys, self.rundata.firstdate, self.rundata.lastdate)
        runid = None
        for run_id_record in sorted(TM.salary_run_id.search(schedule_run_query), key=attrgetter('releasedate', 'starttime'), reverse=True):
...
#                continue
        ...
        if runid == None:
            raise SalaryException("No released shcedule time run was found for period %s - %s." % (self.rundata.firstdate, self.rundata.lastdate))
