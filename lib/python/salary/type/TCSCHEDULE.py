...
from salary.TcSchedule import TcScheduleManager
from utils.fmt import NVL
from AbsTime import AbsTime
from tm import TM

class TcScheduleRun(run.GenericRun):
    def get_codes(self):
    ...
    def rosters(self):
        ...
        try:
            rosters = TcScheduleManager(self.rundata, conf.context, salary_iterator).getScheduleRosters()
        finally:
...
class S3(TcScheduleRun):
    def __init__(self, record):
        articles = ['SCHEDULE_HOURS']
        TcScheduleRun.__init__(self, record, articles)

    def SCHEDULE_HOURS(self, rec):
        return [ self.hours100(x) for x in rec.getScheduledHours() ]

    def __str__(self):
        return 'Swedish schedule for temporary crew (S3)'
