...
from salary.ScheduleCCSE import ScheduleCCSEManager
from utils.fmt import NVL
from AbsTime import AbsTime
from tm import TM

class ScheduleCCSERun(run.GenericRun):
    def get_codes(self):
    ...
    def rosters(self):
        ...
        try:
            rosters = ScheduleCCSEManager(self.rundata, conf.context, salary_iterator).getScheduleRosters()
        finally:
...
class S3(ScheduleCCSERun):
    def __init__(self, record):
        articles = ['SCHEDULE_HOURS']
        ScheduleCCSERun.__init__(self, record, articles)

    def SCHEDULE_HOURS(self, rec):
        return [ self.hours100(x) for x in rec.getScheduledHours() ]

    def __str__(self):
        return 'Swedish schedule for all crew (S3)'
