...
def main():
    ...
    now, entitled_str, correction_str, round_str = R.eval(
        'fundamental.%now%',
        'report_account.%f36_entitled_str%',
        'report_account.%f36_correction_str%',
        'report_account.%f36_round_str%',
    )
    ...
    month_reduction_strings = R.eval(*['report_account.%%f36_reduction_str_built%%(%s)' % m for m in months])
    month_incremented_expr = "add_months(%s, fundamental.%%py_index%% - 1)" % year_start

    crews, = R.eval(
        "sp_crew",
        R.foreach(
            R.iter("iterators.roster_set", where="crew.%is_cabin%"),  # , sort_by="crew.%id%"
            "crew.%id%",
            "crew.%fullname%",
            ...
            R.foreach(
                R.times(12),
                "report_account.%%f36_quota_at_date%%(%s)" % month_incremented_expr,  # is for year, not month.
                'report_account.%%reducing_days_in_month%%(%s, "%s")' % (month_incremented_expr, ACCOUNT),
                'report_account.%%f36_reduction_in_month%%(%s)' % month_incremented_expr,
                'report_account.%%account_sum_month%%(%s, "%s", report_account.%%f36_reduction_str_built%%(%s))' % (
    ...
    if is_daemon:
        ...
        st.show_file_in_window("F36 corrections report", out_file)
