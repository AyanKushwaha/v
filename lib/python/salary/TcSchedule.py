...
from salary.Activity import TcSchedActivityManager
...
class TcScheduleManager:
    ...
    def __init__(self, rundata, context, iterator='iterators.roster_set', crewlist=None):
        ...
        self.activityManager = TcSchedActivityManager(self.context, rundata, self.startDate, self.endDate)
    ...
    def getScheduleRosters(self):
        ...
        while(dt < self.endDate):
            hour_values.append("report_overtime.%%crew_scheduled_hours_no_rounding%%(%s)" % dt)
            dt = dt.adddays(1)
    ...
    def createRoster(self, rosterItem):
        ...
        if includeThisCrew:
            ...
            scheduleRoster = TcScheduleRoster(rosterItem[DUTIES],
                                            rosterItem[CREW_ID],
...
class TcScheduleRoster(DataClass):
    """
...
#                continue
        ...
        if runid == None:
            raise SalaryException("No released schedule time run was found for period %s - %s." % (self.rundata.firstdate, self.rundata.lastdate))
