...
from salary.Activity import BudgetedActivityManager
...
class BudgetedManager:
    """
    A class that creates and holds BudgetedRosters.
    """
    ...
    def __init__(self, rundata, context, iterator='iterators.roster_set', crewlist=None):
        ...
        self.activityManager = BudgetedActivityManager(self.context, rundata, self.startDate, self.endDate)


    def getBudgetedRosters(self):
        budgetedRosters = []
        ...
        while(dt < self.endDate):
            hour_values.append("report_overtime.%%crew_budgeted_hours%%(%s)" % dt)
            dt = dt.adddays(1)
        ...
        for rosterItem in rosters:
            for r in rosterItem:
                if self.crewlist is None or str(r) in self.crewlist:
                    ...
                    if r != None:
                        budgetedRosters.append(r)
                    break

        return budgetedRosters

    def createRoster(self, rosterItem):
        rosterItem = rosterItem[1:]

        budgetedHours = list(rosterItem[BUDGETED_HOURS:])
        ...
        if (len(activities) == 0):
            ...
            if len(allActivities) == 0:
                print "WARNING! No activities for crew %s: skipping schedule. (is_temporary %s rank %s)." % (rosterItem[CREW_ID], rosterItem[IS_TEMPORARY], rosterItem[RANK])
        ...
        if includeThisCrew:
            for (code, extent, startTime, endTime) in activities:
                ...
                while startTime < endTime:
                    ...
                    budgetedHours[offset] = RelTime(0)
                    startTime = startTime.adddays(1)


            budgetedRoster = BudgetedRoster(rosterItem[DUTIES],
                                            rosterItem[CREW_ID],
        ...
        return budgetedRoster
        

class BudgetedRoster(DataClass):
    """
    A Roster item with budgeted values.
    """
    ...
    def getBudgetedHours(self):
        return self.budgetedHours


class BudgetedRunDataHandler:
    def __init__(self, rd):
        self.rundata = rd
        self.budgetedRunData = self._getBudgetedRun()

    def _getBudgetedRun(self):
        budgeted_run_query = "(&(runtype=BUDGETED)(admcode=N)(extsys=%s)(firstdate=%s)(lastdate=%s))" %(self.rundata.extsys, self.rundata.firstdate, self.rundata.lastdate)
        runid = None
        for run_id_record in sorted(TM.salary_run_id.search(budgeted_run_query), key=attrgetter('releasedate', 'starttime'), reverse=True):
            print "Using budgeted time runid %s" % run_id_record.runid
...
#                continue
        ...
        if runid == None:
            raise SalaryException("No released budgeted time run was found for period %s - %s." % (self.rundata.firstdate, self.rundata.lastdate))

        return getDailyRecordsFor(runid)

    def getBudgetedRunData(self):
        return self.budgetedRunData

    def getLastScheduledDay(self, crewid):
        try:
            maxday = 0
            for (day, value) in self.budgetedRunData[crewid]['000']: # BUDGETED_HOURS => 000:
                if day > maxday:
                    maxday = day
