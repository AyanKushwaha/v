...
# exports ================================================================{{{1
__all__ = ['create_and_negate', 'export', 'report', 'run']
...
class GenericRun(object):
    ...
    def run(self):
        ...
        if self.rundata.admcode == 'C':
            self.cancel()
        elif self.rundata.admcode == 'N':
            self.normal()
        elif self.rundata.admcode == 'R':
            self.retro()
            fail_msg = "The Retro run for %s failed, there were no differences compared to the original run." % (self)
    ...
    def cancel(self):
        """Create 'cancel' run."""
        for rec in api.getRecordsFor(self.rundata.runid):
            self.data.append(rec.crewid.id, rec.extperkey, rec.extartid, -int(rec.amount))
        for rec in api.getExtraRecordsFor(self.rundata.runid):
            self.extradata.append(rec.crewid.id, rec.intartid, -int(rec.amount))
    ...
    def retro(self):
        """ Create a 'retro' run by subtracting values from an earlier run to
        get the deltas in the current run. 
        NOTE: Overridden in type/PERDIEM.py for Swedish crew."""
        ...
        while r_runid != None:
            for rec in api.getRecordsFor(r_runid):
                key = (rec.extperkey, rec.extartid, rec.crewid.id)
                if key in r_run:
                    r_run[key] += rec.amount
                else:
                    r_run[key] = rec.amount
            local_rundata = api.getRunIdData(r_runid)
            if local_rundata.admcode.admcode == 'R':
                r_runid = int(local_rundata.selector)
            else:
                r_runid = None

        for r in self.data:
            key = (r.extperkey, r.extartid, r.crewid)
            if key in r_run:
                r.amount -= r_run[key]
                del r_run[key]

        # It's always tricky to remove records from a list, so we'll just create a new one.
        # This method will remove records with amount == 0.
        odata = self.data
        self.data = BasicData()
        for d in odata:
            self.data.append(d.crewid, d.extperkey, d.extartid, d.amount)

        # Now, append all remaining records from r_run (with negative sign)
        for key in r_run:
            extperkey, extartid, crewid = key
            self.data.append(crewid, extperkey, extartid, -r_run[key])

        # Get run id of run to reverse from 'selector' field.
        r_runid = int(self.rundata.selector)

        # (II) salary_extra_data
        r_extra_run = {}

        # Follow retro trail
        while r_runid != None:
            for rec in api.getExtraRecordsFor(r_runid):
                key = (rec.crewid.id, rec.intartid)
                if key in r_extra_run:
                    r_extra_run[key] += rec.amount
                else:
                    r_extra_run[key] = rec.amount
            local_rundata = api.getRunIdData(r_runid)
            if local_rundata.admcode.admcode == 'R':
        ...
        for r in self.extradata:
            key = (r.crewid, r.intartid)
            if key in r_extra_run:
                r.amount -= r_extra_run[key]
                del r_extra_run[key]

        # It's always tricky to remove records from a list, so we'll just create a new one.
        # This method will remove records with amount == 0.
        oextradata = self.extradata
        self.extradata = ExtraData()
        for d in oextradata:
            self.extradata.append(d.crewid, d.intartid, d.amount)

        # Now, append all remaining records from r_run (with negative sign)
        for key in r_extra_run:
            crewid, intartid = key
            self.extradata.append(crewid, intartid, -r_extra_run[key])
...
class RunData(dict):
    ...
    _fieldlist = ('admcode', 'exportformat', 'extsys', 'firstdate', 'lastdate',
                  'note', 'runid', 'runtype', 'selector', 'spooldir', 'starttime', 'fromStudio', 'monthsBack', 'noCheckPP')
    ...
    @classmethod
    def fromTempRec(cls, rec):
        """ Fill with values from a temporary table (tmp_salary_run_id). """
        return cls(
            admcode=rec.admcode_expl.admcode,
            extsys=rec.extsys.extsys,
            note=rec.note,
            lastdate=rec.lastdate,
            firstdate=rec.firstdate,
            starttime=rec.starttime,
            runtype=rec.runtype_expl.runtype,
            selector=rec.selector
        )

    # Alternate initialization
    @classmethod
    def fromTableRec(cls, rec):
        """ Fill with values from the salary_run_id table (persistent table). """
        return cls(
            runid=rec.runid,
            admcode=rec.admcode.admcode,
            extsys=rec.extsys,
            note=rec.note,
            lastdate=rec.lastdate,
            firstdate=rec.firstdate,
    ...
    # Alternate initialization
    @classmethod
    def fromRunId(cls, runid):
        """ Fill with values from salary_run_id for given runid. """
        return cls.fromTableRec(api.getRunIdData(int(runid)))
...
class RunFiles:
    ...
    def __init__(self, r, is_release=False):
        ...
        if is_release and self.rundata.extsys == 'S3' and self.rundata.runtype == 'ABSENCE':
            st = time.strptime(str(self.rundata.starttime), '%d%b%Y %H:%M')
            self.name = os.path.expandvars("$CARMDATA/REPORTS/SALARY_RELEASE/absence_cms_se_%04d%02d%02d_1.dat" % (st.tm_year, st.tm_mon, st.tm_mday))
        elif is_release and self.rundata.extsys == 'S3' and self.rundata.runtype == 'SCHEDULE':
            st = time.strptime(str(self.rundata.starttime), '%d%b%Y %H:%M')
            self.name = os.path.expandvars("$CARMDATA/REPORTS/SALARY_RELEASE/schedule_cms_se_%04d%02d%02d_1.dat" % (st.tm_year, st.tm_mon, st.tm_mday))
        elif is_release and self.rundata.extsys == 'S3' and self.rundata.runtype == 'TCSCHEDULE':
            st = time.strptime(str(self.rundata.starttime), '%d%b%Y %H:%M')
            self.name = os.path.expandvars("$CARMDATA/REPORTS/SALARY_RELEASE/schetemp_cms_se_%04d%02d%02d_1.dat" % (st.tm_year, st.tm_mon, st.tm_mday))
        elif is_release and self.rundata.extsys == 'S3' and self.rundata.runtype == 'SCHED_CCSE':
            st = time.strptime(str(self.rundata.starttime), '%d%b%Y %H:%M')
            self.name = os.path.expandvars("$CARMDATA/REPORTS/SALARY_RELEASE/scheccse_cms_se_%04d%02d%02d_1.dat" % (st.tm_year, st.tm_mon, st.tm_mday))
        elif is_release and self.rundata.extsys == 'S3' and self.rundata.runtype == 'PERDIEM':
            st = time.strptime(str(self.rundata.starttime), '%d%b%Y %H:%M')
        ...
        elif is_release and self.rundata.extsys == 'S3' and self.rundata.runtype == 'ALLOWNCE_M':
            st = time.strptime(str(self.rundata.starttime), '%d%b%Y %H:%M')
            self.name = os.path.expandvars("$CARMDATA/REPORTS/SALARY_RELEASE/allowances1_cms_se_%04d%02d%02d_1.dat" % (st.tm_year, st.tm_mon, st.tm_mday))
        elif is_release and self.rundata.extsys == 'S3' and self.rundata.runtype == 'ALLOWNCE_D':
            st = time.strptime(str(self.rundata.starttime), '%d%b%Y %H:%M')
            self.name = os.path.expandvars("$CARMDATA/REPORTS/SALARY_RELEASE/allowances2_cms_se_%04d%02d%02d_1.dat" % (st.tm_year, st.tm_mon, st.tm_mday))
        elif is_release and self.rundata.extsys == 'S3' and self.rundata.runtype == 'VACATION_Y':
            st = time.strptime(str(self.rundata.starttime), '%d%b%Y %H:%M')
            self.name = os.path.expandvars("/opt/Carmen/CARMTMP/ftp/out/vacrenum_cms_se_%04d%02d%02d_1.dat" % (st.tm_year, st.tm_mon, st.tm_mday))
        else:
...
# create_and_negate ------------------------------------------------------{{{2
def create_and_negate(rundata):
    """
    Read records from a run, copy it to a new run with all amounts negated.
    """
    log.debug("create_and_negate(%s)" % (rundata))

    # Update RunId and RunDate, set admin code to 'C' == 'Cancelled'
    # [acosta:08/150@14:04] BZ 27961
    rd = RunData(**rundata)
    rd.admcode = 'C'
    if rundata.note is None:
        rd.note = 'Cancellation of %s.' % rundata.runid
    else:
        rd.note = 'Cancellation of %s. [%s]' % (rundata.runid, rundata.note)
    rd.starttime = Times().now
    return run(rd)
...
def export(rundata, is_release=False):
    ...
    call_export_daily_for = ("ABSENCE", "ALLOWNCE_D", "SCHEDULE", "TCSCHEDULE", "SCHED_CCSE")
    if (rundata.extsys == "S2") or (rundata.extsys == "S3" and rundata.runtype in call_export_daily_for):
        return export_daily(rundata, is_release)
...
def export_daily(rundata, is_release=False):
    """Export to file and return file name."""
    run_types_storing_extartids_to_db = ('BUDGETED', 'ABSENCE', 'SCHEDULE', 'TCSCHEDULE', "SCHED_CCSE", 'ALLOWNCE_D')
    log.debug("export(%s)" % rundata)
    ...
    daily = api.getDailyRecordsFor(rundata.runid)

    if rundata.runtype not in run_types_storing_extartids_to_db:
        # collect all intartids used in this run
        intartids = []
        for _, articles in daily.iteritems():
            for intartid, _ in articles.iteritems():
                if intartid not in intartids:
                    intartids.append(intartid)

        articles = api.getArticleIds(intartids, rundata.extsys, rundata.starttime)
        # A dangerous part. We will get an exception here if some extartid is used for more
        # than one intartid. Improve this later.
        articles = dict((v, k) for k, v in articles.iteritems())

    # iterating over records from the salary_basic_data table
    for rec in api.getRecordsFor(rundata.runid):
        extartid_list = rec.extartid.split(':')
        extartid = extartid_list[0]

        if rundata.runtype not in run_types_storing_extartids_to_db:
            for amount in daily[rec.crewid.id][articles[extartid]]:
                exp.write(rec.extperkey, rec.extartid, amount)
        else:
            try:
                exp.write(rec.extperkey, rec.extartid, daily[rec.crewid.id][extartid])
            except KeyError:
                print "crewid: %s extperkey: %s extartid: %s daily[crewid] %s" % (rec.crewid.id, rec.extperkey, rec.extartid, daily[rec.crewid.id])
                raise
...
def release(rundata):
    ...
    if rundata.extsys not in ('CN', 'JP', 'HK'):
        ...
        # (3) Send email with 'Afstemningsunderlag'
        if not (rundata.extsys == 'S3' and rundata.runtype in ('SCHEDULE', 'TCSCHEDULE', "SCHED_CCSE")):
            reports.append({
                'content-type': 'text/plain',
                'content': report(rundata),
                'destination': [("BALANCING_REPORT", {'subtype':rundata.runtype, 'rcpttype':rundata.extsys})],
            })

        # SASCMS-3074: Send Resource Pool Illness Report for Swedish crew.
        if rundata.extsys in ('SE') and rundata.runtype == 'TEMP_CREW':
            filename = "RESOURCE_POOL_ILLNESS_%08d.pdf" % (rundata.runid)
            reports.append({
                'content-type': 'application/pdf',
                'content-location': os.path.join(api.reportDirectory(), filename),
                'destination': [("RESOURCE_ILL_REPORT", {'subtype':rundata.runtype, 'rcpttype':rundata.extsys})],
            })
...
# retro_run --------------------------------------------------------------{{{2
def retro_run(rundata):
    """
    Create a "retro" run. Note that the logic is actually contained in the
    class 'GenericRun', method 'retro'.
    Return runid of the newly created run.
    """
    log.debug("retro_run(%s)" % (rundata,))
    rd = RunData(**rundata)
    # Set type to 'R' (Retro)
    rd.admcode = 'R'
    if rundata.note is None:
        rd.note = 'Retro of %s.' % rundata.runid
    else:
        rd.note = 'Retro of %s. (%s)' % (rundata.runid, rundata.note)
    rd.starttime = Times().now
    # Put the run Id in the selector string instead.
    rd.selector = str(rundata.runid)
    # Now when input data is primed, start the real run.
    run(rd)
    return rd.runid
...
def run(rundata):
    ...
    'salary.type.PERDIEM.DK(rundata).run()' will be called.
...
# send_balancing ---------------------------------------------------------{{{2
def send_balancing(rundata, recipient):
    """Send balancing report (Afstemningsunderlag) to recipient. This function
    is called from the Salary MMI when requesting a 'Balancing Report'."""
    log.debug("send_balancing(%s, %s)" % (rundata, recipient))
    return [{
        'content-type': 'text/plain',
        'content': report(rundata),
        'destination': [
            ('mail', {
                'to': "%s" % recipient, 
                'mfrom': conf.balancing_report_sender,
                'subject': '[CMS] %s' % (conf.balancing_report_subject_fmt % rundata.runid),
            })],
    }]
