...
from salary.ECPerDiem import PerDiemRosterManager
from salary.ECSupervis import SupervisRosterManager
from salary.ECOvertime import OvertimeRosterManager
...
salary_perdim_article = {
    ...
    'NO': ['MEAL_C', 'MEAL_F', 'PERDIEM_SALDO', 'PERDIEM_TAX_DAY', 'PERDIEM_TAX_DOMESTIC'],
...
salary_overtime_article = {
    'DK': ['SCC', 'SNGL_SLIP_LONGHAUL'],
    'NO': ['SCC', 'MDCSH', 'SCCNOP', 'MDCLH', 'SNGL_SLIP_LONGHAUL'],
...
REPORT_PATH = CARMDATA + "/REPORTS/SALARY_EC/"
...
class ECReport():
    def __init__(self, salary_system='all', report_start_date=None, report_end_date=None, release=True, test=False):
        self.salary_system = salary_system
        self.release = release # Is it required or deprecated
        self.test = test
        if report_start_date:
            # Expected format is like "10Nov20"
            self.start = datetime.strptime(report_start_date,"%d%b%Y")
        else:
    ...
    def generate(self, crew_ids=[]):
        ...
        runs.append(PerDiemRun(salary_system=self.salary_system, crew_ids=crew_ids, salary_article_tm=self.salary_article_tm, \
            report_first_absdate=self.firstdate, report_last_absdate=self.lastdate, release=self.release, test=self.test).run())
        ...
        createCSV(entries, self.release)
            ...
            if crew.salarySystem in salary_systems.keys():
                for a in salary_perdim_article[crew.salarySystem]:
                    ...
                    if value is not None and int(value) != 0:
                        entries[crew.salarySystem].append((crew.homeCurrency, self.start.strftime('%d/%m/%Y'), self.salary_article_tm[crew.salarySystem][a], crew.empNo, value/ 100.0, '', '', ''))
...
class SupervisRun(ECGenericRun):
    ...
    def INST_LPC_OPC_OTS(self, rec):
        return hours100(rec.lpc_opc_ots)

    def INST_LPC_OPC_OTS_BD(self, rec):
        return times100(rec.lpc_opc_ots_bd)
...
def createCSV(entries, release, filename_prefix='Payments_CMS'):
    csv_files = []
    ...
    for co in entries.keys():
        if len(entries[co]) > 0:
            ...
            log.debug("{0} => length: {1} =>=> first entry:{2}".format(co, len(entries[co]), entries[co][0]))
            with open(REPORT_PATH + csv_file, 'w') as f:
                ...
                for (homeCurrency, start, article, empNo, value, n, units, o) in entries[co]:
                    ...
                    # NO => N3150
                    if co == 'DK' and article in ['1540', '1548', '1550'] and article[0] != 'D':
                        article = 'D' + article
