...
"""
Per Diem Statem ent Report for Employee Central.
"""
...
import salary.ec.ECPerDiem as pd
...
class ECPerDiemStatementReport(SASReport):
    """
    Arguments to report:
    ...
    def create(self):
        ...
        if context is None:
            ...
            if retrofile is not None:
                ...
                f.close()
                title = 'Revised EC Per Diem Statement'
        ...
        old_salary_month_start = R.param(startparam).value()
        R.param(startparam).setvalue(firstdate)
        old_salary_month_end = R.param(endparam).value()
        R.param(endparam).setvalue(lastdate)
        try:
            if rtype == 'roster':
            ...
            R.param(startparam).setvalue(old_salary_month_start)
            R.param(endparam).setvalue(old_salary_month_end)
...
def runReport(scope='window'):
    ...
    try:
        start_date = R.param('parameters.%ECPerDiemStatement_report_start_date%').value()
        end_date = R.param('parameters.%ECPerDiemStatement_report_end_date%').value()
    except:
        start_date = None
        end_date = None
    else:
    ...
    rptForm = display.reportFormDate('Employee Central Per Diem Statement', start_date=start_date, end_date=end_date)
    rptForm.show(True)
    if rptForm.loop() == Cfh.CfhOk:
        nowTime, = R.eval('fundamental.%now%')
        R.param('parameters.%ECPerDiemStatement_report_start_date%').setvalue(AbsTime(rptForm.getStartDate()))
        R.param('parameters.%ECPerDiemStatement_report_end_date%').setvalue(AbsTime(rptForm.getEndDate()))
        ...
        Cui.CuiCrgDisplayTypesetReport(Cui.gpc_info, area, scope,
            '../lib/python/report_sources/include/ECPerDiemStatementReport.py', 0, args)
