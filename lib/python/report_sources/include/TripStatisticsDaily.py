...
GENERAL_KEYS = ['Night Stops', 'Day stops', 'AC legs', 'Active legs',
                ...
                '2 Day Trips', '3 Day Trips', '4 Day Trips', '5 Day Trips']
...
# PairingStatsDaily----------------------------------------------------------{{{2
class TripStatisticsDaily(SASReport):
    """
    ...
    """
    def create(self, outputType = 'standard'):
        ...
        # Removed SKCMS-1996
        # Push report end to include any days of trips starting in the pp
        # last_trip_end = R.eval('crg_trip.%last_trip_end_day_safe%')[0]
        # if last_trip_end > pp_end:
        #    pp_end = last_trip_end
        ...
        if self.dated:
            ...
            for date in dates:
                ...
                if week not in found_weeks:
                    self.weeks.append((week, date))
                    found_weeks.append(week)

            for w in range(len(self.weeks)):
                (week, start_date) = self.weeks[w]
                try:
                    (next_week, next_start_date) = self.weeks[w + 1]
                except IndexError:
                    next_start_date = dates[-1] + RelTime(24, 0)
                self.weeks[w] = (week, start_date, next_start_date - RelTime(24, 0))

        # Add the entire period as the last "week", not including
        # the extra date added if the number of dates are odd
        if odd_dates:
            self.weeks.append((0, self.dates[0], self.dates[-2]))
        else:
            self.weeks.append((0, self.dates[0], self.dates[-1]))
        ...
        fe = R.foreach(R.iter('iterators.trip_set'),
                       'trip.homebase',
        ...
        for (trip_ix, base, trip_start_day, trip_start_weekday, trip_days,
             trip_assigned, is_long_haul, night_stops, day_stops, ac,
             active_legs, passive_legs, extra_seats, one_day_trip, two_day_trip,
             three_day_trip, four_day_trip, five_day_trip, freedays, duties) in trips:
            ...
            for (jx, no_of_crew, req_freedays) in freedays:
            ...
            ## Iterate over duties
            for (duty_ix, duty_start_day, duty_start_weekday,
                ...
                for (leg_ix, leg_key, leg_time, leg_is_deadhead) in legs:
                    if leg_key in added_flights:
                        ...
                        # Add zero to ensure the datacollection is correct
                        elif not KEY_AC_BLOCK in dt.keys():
                            dt.add(KEY_AC_BLOCK, duty_start_day, RelTime(0, 0))
    ...
    def buildSummaryBox(self, base, weeks = None): #fromDate = None, toDate = None):
        ...
        for key in loop_keys:
            ...
            key_values = []
            space = ''
            for (week, fromDate, toDate) in weeks:
                if key == KEY_AV_BLOCK:
    ...
    def sumData(self):
        ...
        for base in self.data:
            sum.addDataCollection(self.data[base])
        self.data['All bases'] = sum
