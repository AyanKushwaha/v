...
# Used to make it possible to export parings and make it possible to measure crew and AC cnx.create reports in common applikations.    
...
class PairingsOutput(p.Report):
    def create(self):
        if 'CMSDEV' in os.getenv("CARMSYSTEMNAME", ""):
           username = os.environ.get("USER")
           Homepath = "/home/" + username
        else:
           samba_path = os.getenv('SAMBA', "/samba-share")
           Homepath = samba_path + "/reports/PairingsOutput"
...
#A. Imported values from Rave code. To be given name and used later in the report. Name must be given in the same order as they are imported.
        L_VALUES = ['trip.%start_hb%','trip_id','assigned_crew_position_1','assigned_crew_position_2','assigned_crew_position_3','assigned_crew_position_5','assigned_crew_position_6',
                    ...
                    'aircraft_change','leg.%block_time%','leg.%region%','trip.%days%','trip_cost.%trip_real_costs%','trip_cost.%trip_soft_costs%','fdp.%time%','oma16.%max_daily_fdp%',
                    ...
                    'leg.%is_last_in_duty%','trip.%homebase%','rest.%minimum_time%','crg_info.%_rest_time%','report_meal.%has_meal_code%']
...
#A. Valuenames for all Rave variables previously imported in this report. Must be in the same order as: "A. Imported values.."      
        for (ix, date, tripId, fc, fp, fr, ap, AS, ah, dep_time, dep_stn, flt_nbr, dh, ac, arr_time, arr_stn,cnx, ac_cng, BLH, 
             region, tripLen, pair_cost, post_pair_cost, FDP, maxFDP, is_extension, duty_in_duty, duty_in_trip, last_in_fdp, 
             last_leg_in_trip, first_leg_in_trip, last_leg_in_duty, base, minRest, actRest, has_meal_code) in trip:
...
#B. Header name for each column in the outputfile
                L = ["TripStartLocalHB","Trip_ID","FC","FP","FR","AP","AS","AH","DepUTC","DepStn","FltNr","Deadhead",
                     "AcType","ArrUTC","ArrStn","Cnx","AC_CNG","BLH","LegRegion","MealCode","DutyInDuty","FDP","MaxFDP","Extension",
