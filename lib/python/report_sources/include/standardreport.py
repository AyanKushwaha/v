...
from carmstd import date_extensions as de
from carmstd import log


def format_num(value, form="%d"):
    '''
    Formats a number float or integer or decimal according to the set locale.

    LC_NUMERIC in the config extension file decides the separation
    @param value: Value to format
    @type value: int
    @param form: String type decimal or float 2 digits after comma, "%d" is default.
    @type form: %d, %.2f
    '''
...
MONOSPACE_FONT = prt.font(face=prt.MONOSPACE)
...
class StandardReport(prt.Report):
    ...
    def __del__(self):
        cls = self.__class__
        log.info("\n".join(("*********************************************************************",
                            "An instance of '%s.%s' has been deleted." % (cls.__module__, cls.__name__),
                            "*********************************************************************")))
    ...
    def store_bag(self):
        ...
        # Store the current area context for interaction
        ...
        if self.get_scope() == "plan":
            ...
            if self.get_type() == "current_roster":
                self.bag_wrapper = bag_handler.CurrentChain()
    ...
    def standard_header(self):
        ...
        *REPORT TITLE*                                   *OTS LOGO*
        ...
        Plan:                TT / BASIC_JCP / Dated_1 / HotelStatistics
        ...
        pp_start = ctx_bag.report_common.report_pp_start()
        pp_end = ctx_bag.report_common.report_pp_end()
...
class SimpleTable(prt.Column):
    ...
    """
    def __init__(self, title, cross_name='', use_page=True, image=None):
        """
...
class SimpleCalendar(SimpleTable):
    ...
    def __init__(self, title, cross_name='', report_type='weekly'):
        ...
        if report_type == 'weekly':
            self.add_sub_title('')
            self.add_sub_title(de.day_num2gui_weekday_short_name(1))
            self.add_sub_title(de.day_num2gui_weekday_short_name(2))
            self.add_sub_title(de.day_num2gui_weekday_short_name(3))
            self.add_sub_title(de.day_num2gui_weekday_short_name(4))
            self.add_sub_title(de.day_num2gui_weekday_short_name(5))
            self.add_sub_title(de.day_num2gui_weekday_short_name(6))

            self.add_sub_title(prt.Text(de.day_num2gui_weekday_short_name(7),
                                        colour=studio_palette.Sunday))
        else:
            log.error('standardreport.SimpleCalender :: only "weekly" tables are supported, returning empty table')


def abs2hhmm(a):
    """
    method to format an AbsTime into HH:MM (08:42)
...
def abs2dhm(r):
    """
    method to format a RelTime into DdHhM (2d8h42)
    @param a: time to be formated
    @type a: RelTime
    ...
    h = raw_h % 24
    if d:
        ret = MSGR('{0:d}d{1:d}h{2:02d}').format(d, h, m)
    else:
        ret = MSGR('{0:d}h{1:02d}').format(h, m)
    return ret
