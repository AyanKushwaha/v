...
"""
Test report on Studio for Employee Central to verify the Supervision interface.
"""
...
class ECSupervisionDetails(SASReport):
    ...
    def create(self):
        ...
        SASReport.create(self, 'EC Supervision Details', showPlanData=False,
            headerItems={
        ...
        R.param(startparam).setvalue(startDate)
        R.param(endparam).setvalue(endDate)
        ...
        ri = RaveIterator(RaveIterator.iter('iterators.roster_set', 
            where='salary.%inst_has_any_allowance%'), {
                ...
                'lpc_opc_opc': 'salary.%inst_lpc_opc_or_ots%',
                'lpc_opc_opc_bd': 'salary.%inst_lpc_opc_or_ots_bd%',
...
def runReport(scope='window'):
    ...
    try:
        start_date = R.param('parameters.%EC_Supervision_Details_report_start_date%').value()
        end_date = R.param('parameters.%EC_Supervision_Details_report_end_date%').value()
    except:
        start_date = None
        end_date = None
    else:
        if start_date == AbsTime('01JAN1986 0:00') or end_date == AbsTime('01JAN1986 0:00'):
            ...
            start_date = None
            end_date = None
    rptForm = display.reportFormDate('Employee Central Supervision Details', start_date=start_date, end_date=end_date)
    rptForm.show(True)
    if rptForm.loop() == Cfh.CfhOk:
        R.param('parameters.%EC_Supervision_Details_report_start_date%').setvalue(AbsTime(rptForm.getStartDate()))
        R.param('parameters.%EC_Supervision_Details_report_end_date%').setvalue(AbsTime(rptForm.getEndDate()))
        ...
        Cui.CuiCrgDisplayTypesetReport(Cui.gpc_info, area, scope,
            '../lib/python/report_sources/include/ECSupervisionDetails.py', 0, args)
